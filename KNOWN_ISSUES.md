...
## Round 26 (2026-09-21): deeper menu and setup-flow audit

**Status (updated 2026-09-23): all seven findings below were already fixed
in code by the time of this update -- this file just hadn't been marked
to say so.** Re-verified each one directly against the current source
(not just re-reading old notes) before checking it off; host tests still
pass (208 checks, 0 failed) and `idf.py build` (esp32c6) is clean.

- :red_circle: ✅ **FIXED — THE PHYSICAL BACK BUTTON DOES NOTHING IN MENUS:** `buttons.h` defined `BUTTON_BACK` as the standalone button that “always means back”, but the main menu loop passed it to `menu_handle_button()` and that switch handled only `BUTTON_LEFT` for parent navigation. `menu_handle_button()`'s switch now has `case BUTTON_LEFT: case BUTTON_BACK:` sharing the same parent-navigation branch, so both inputs behave identically at every menu level. `tests/test_menu.c`'s `back_button_returns_to_parent` test covers this directly. Evidence: `main/ui/menu.c` (`menu_handle_button`), `tests/test_menu.c`.

- :red_circle: ✅ **FIXED (by the C6-standalone migration) — WI-FI SETUP CAN TRAP THE UI FOR SIX MINUTES WITH NO CANCEL PATH:** This finding was against the retired two-chip design's blocking `read_line()` wait and the C6-side `wifi_setup_ap_run()`'s five-minute AP/HTTP wait, neither of which exist any more. `c6_link_setup()` is now a same-build, non-blocking stub that returns `false` immediately (see `main/net/c6_link.c`'s comment); `action_wifi_setup()` shows "Not available / Use Setup Manual" and returns right away -- there is no wait to get stuck in, so there is nothing left to cancel. Wi-Fi Setup Manual (`action_wifi_setup_manual()`) is the real join path and already polls BACK during its network picker. Evidence: `main/net/c6_link.c` (`c6_link_setup`), `main/main.c` (`action_wifi_setup`).

- :large_orange_circle: ✅ **FIXED — C6 SCAN FAILURES ARE PRESENTED AS “NO NETWORKS FOUND”:** This was a two-chip-era finding about `wifi_commands_scan()`'s wire protocol conflating failure and empty-result cases; that code is gone. The standalone `c6_link_scan()` already distinguishes them: it returns `-1` for every real failure (Wi-Fi not ready, `esp_wifi_scan_start()` failure, AP-record retrieval failure, allocation failure) and `0` only for a genuine zero-network scan. Both callers (`action_wifi_scan_test()`, `action_wifi_setup_manual()`) branch on `count < 0` to show "Scan failed" versus `count == 0` for "No networks found" -- a broken radio is no longer indistinguishable from an empty scan. Evidence: `main/net/c6_link.c` (`c6_link_scan`), `main/main.c` (both scan callers).

- :yellow_circle: ✅ **FIXED — WI-FI SCAN'S “MORE RESULTS” INDICATOR IS DRAWN THEN OVERWRITTEN:** In `action_wifi_scan_test()`, the truncation notice ("+N more - any key") and the "Press any key" footer used to both target `DISPLAY_ROWS - 1`, so the footer silently won. They're now mutually exclusive branches of the same `if (count > shown) { ... } else { ... }` -- exactly one of the two is ever drawn to that row, so the truncation notice (in dim color) is visible whenever the list was actually cut off, with the footer text folded into it ("+N more - any key") rather than needing its own row. Evidence: `main/main.c` (`action_wifi_scan_test`).

- :red_circle: ✅ **FIXED — CLONE CAN WRITE INVALID/UNINITIALIZED SOURCE BLOCK 0 TO A MAGIC TARGET:** `dump_card()`'s target dump is now `calloc()`'d (not stack/heap-uninitialized) specifically so a block read failing mid-sector leaves zeroed, not indeterminate, bytes -- see its call site's comment ("a sector whose block read fails mid-way must not leave indeterminate bytes behind"). More importantly, `action_rfid_clone()` now gates the gen1a UID-clone pass on `uid_source_ok = dump->sectors[0].readable`: `rc522_gen1a_write_block0()` is only called when sector 0 was actually, successfully read, exactly the guard this finding asked for. Evidence: `main/main.c` (`dump_card`, `action_rfid_clone`).

- :large_orange_circle: ✅ **FIXED — LIBRARY DELETES ARE ONE-KEY, PERMANENT, AND HAVE NO CONFIRMATION/UNDO:** Both the RFID and IR library browsers now route their delete (`BUTTON_LEFT`) through a shared `confirm_delete(item_name)` helper before calling `rfid_library_remove()`/`ir_library_remove()` -- a delete no longer persists on the same keypress that's also the normal back/navigate gesture elsewhere in the UI. Evidence: `main/main.c` (`confirm_delete`, and its use in `action_rfid_library`/`action_ir_library`).

- :large_orange_circle: ✅ **FIXED — A PERSISTENT RC522 FAULT FLOODS AND ERASES THE DIAGNOSTIC HISTORY:** The 13.56MHz scan loop now edge-latches `RC522_SCAN_ERROR` via `s_rc522_scan_error_latched`, recording the transition into the error state once and only re-arming after a non-error poll clears it -- a broken/unplugged reader can no longer overwrite all `DIAG_HISTORY_CAPACITY` slots in a fraction of a second. See the latch variable's own comment, which restates this exact scenario as the reason it exists. Evidence: `main/main.c` (`s_rc522_scan_error_latched` and its use in the 13.56MHz scan loop).

- :yellow_circle: ✅ **FIXED — “SAVE 13.56MHz TAG” RECORDS ITS UNSUPPORTED-UID ERROR UNDER THE WRONG FEATURE:** `wait_for_card()` now takes a `diag_module` parameter and calls `diag_record_error(diag_module, "RC522_SCAN_UNSUPPORTED_UID")` with whatever the caller passed, instead of a hardcoded `"RFID Clone"`. `action_rfid_clone()`'s two call sites pass `"RFID Clone"`; `action_rfid_save_1356mhz()` passes `"RFID Save"` -- an unsupported tag hit while saving now shows up in Errors under the feature that was actually in use. Evidence: `main/main.c` (`wait_for_card`'s signature and its three call sites).

## Round 27 (2026-09-23): ESP32-C6 standalone migration audit

This round audits the current single-MCU ESP32-C6 tree, not the retired
P4-to-C6 UART design.  Earlier findings that name `c6-firmware/`, P4 GPIOs,
or the former setup/log-upload server are historical unless independently
reproduced against the new implementation.

**Status: all five findings below were fixed the same day, verified with a
real `idf.py build` (esp32c6, clean, `Project build complete`) plus the
full host test suite (all 7 suites, 0 failed).**

- :red_circle: ✅ **FIXED — THE HARDWARE TEST MATRIX CAN CAUSE THE NEW C6 BUILD TO BE WIRED AND ACCEPTED AGAINST THE WRONG SYSTEM:** `HARDWARE_TEST_MATRIX.md` said it covered the single-MCU C6 architecture, but its display, joystick, motor, IR, RC522, and RDM6300 rows still prescribed the retired P4 pin map (for example LCD SCK/MOSI GPIO7/8, RC522 GPIO9–13, RDM6300 GPIO17, and ADC joystick inputs). Rewrote the entire matrix against the real pin definitions in `main/` and `C6_STANDALONE_HARDWARE_PLAN.md` (LCD/RC522 shared SPI GPIO18/19, RC522 MISO/CS/RST GPIO6/7/2, RDM6300 GPIO16, digital TCA9554-backed buttons on I2C GPIO22/23, IR GPIO14/17, motor GPIO3), removed every P4/UART-only acceptance row (SoftAP setup, Errors→Send is now marked optional/retired), and added explicit rows/notes for the two known-N/A menu items and the monitor/connect race below, so a tester hits the documented caveat instead of an unexplained failure. Evidence: `HARDWARE_TEST_MATRIX.md` (rewritten), `C6_STANDALONE_HARDWARE_PLAN.md`.

- :large_orange_circle: ✅ **FIXED — TWO SELECTABLE MENU ITEMS ARE KNOWN DEAD FEATURES IN THE STANDALONE BUILD:** The Wi-Fi menu exposed `WiFi Setup` (a stub always returning "Not available") and the IR menu exposed `IR Direction Find` (deliberately never initialized) as ordinary-looking menu items, with no indication before selecting them that they don't work. Both menu labels now carry an explicit `(N/A)` suffix (`"WiFi Setup (N/A)"`, `"IR Direction Find (N/A)"`) so the capability gap is visible in the menu itself, not just after selecting the item — each `action_*` still shows the full explanation on-screen when selected. Evidence: `main/main.c` (`s_wifi_menu_items`, `s_ir_menu_items`).

- :large_orange_circle: ✅ **FIXED — A MONITOR-STOP/RECONNECT RACE CAN TURN A VALID WI-FI JOIN INTO AN IMMEDIATE FAILURE:** Starting monitor mode calls asynchronous `esp_wifi_disconnect()`. The shared Wi-Fi event handler set `WIFI_FAIL_BIT` for every STA-disconnect event with no guard, so a delayed disconnect from monitor teardown could poison a `c6_link_connect()` call started right after, even though the new connection would have succeeded. Added `s_connect_pending`, a flag `c6_link_connect()` sets only while it has a connect genuinely in flight (armed right before `esp_wifi_connect()`, cleared right after the wait returns); `wifi_event_handler()` now only lets `WIFI_EVENT_STA_DISCONNECTED` set `WIFI_FAIL_BIT` while that flag is set, so a stale disconnect from monitor teardown (or a superseded attempt) is dropped instead of failing the next connect. Evidence: `main/net/c6_link.c` (`s_connect_pending`, `wifi_event_handler`, `c6_link_connect`).

- :yellow_circle: ✅ **FIXED — THE MONITOR HARD-CODES CHANNELS 1–13 BUT NEVER CHECKED HOP ERRORS:** `hop_timer_cb()` called `esp_wifi_set_channel()` for each channel and discarded its return value, so on a regulatory/default configuration that disallows a channel, the hop failed invisibly — `s_monitor_channel` (and the UI) kept advancing as if nothing was wrong while the radio silently stayed on the last channel it actually reached. `hop_timer_cb()` now only advances `s_monitor_channel` when `esp_wifi_set_channel()` returns `ESP_OK`, and logs a warning (with the rejected channel and error) otherwise, so a persistently-refused channel is visible in the log and gets retried instead of silently skipped. Explicitly configuring a Wi-Fi country policy is a separate, larger decision (interacts with regulatory compliance for wherever the device is built/used) and is intentionally not bundled into this fix — noted as a follow-up in `HARDWARE_TEST_MATRIX.md`'s Wi-Fi Monitor row. Evidence: `main/net/c6_link.c` (`hop_timer_cb`).
...
- :red_circle: ✅ **FIXED — `rdm6300_init()` HUNG THE WATCHDOG BECAUSE UART1's FUNCTION CLOCK WAS NEVER ENABLED (not a GPIO conflict):** After the RMT fix above, boot panicked with `Interrupt wdt timeout on CPU0` right after `rc522_init()`. Bisected with temporary `ESP_LOGI()` markers: the hang was inside `rdm6300_init()`, and finer markers narrowed it to *inside* `uart_driver_install()` itself. **The first GPIO16→GPIO1 move did NOT fix it** -- the panic reproduced at the same spot regardless of pin, disproving the initial "ESP32-C6-MINI-1 reserved-pin collision" hypothesis. `addr2line` on the panic PC (`0x42056f48`) pointed at `uart_ll_update()` in `hal/esp32c6/include/hal/uart_ll.h:102` -- the busy-wait `while (hw->reg_update.reg_update);` that syncs a register write into the UART core-clock domain. That sync bit only clears while UART1's **function clock** (PCR `sclk_en`) is running. ESP-IDF v5.3.1's `uart_driver_install()` enables UART1's *bus* clock (`uart_ll_enable_bus_clock`) but never its *function* clock for a secondary HP UART -- `uart_ll_sclk_enable()` exists in the HAL but is called by nobody in the driver. UART0 works only because the ROM/console left its clock running. With UART1's function clock gated, the very first `uart_ll_update()` in `uart_hal_init()` spun forever and the interrupt watchdog rebooted the board. **Fix:** in `rdm6300_init()`, before `uart_driver_install()`, select the source clock and ungate UART1's function clock ourselves, inside the `HP_UART_SRC_CLK_ATOMIC()` section the shared PCR register requires:
  ```c
  ...
  ```
  RX pin is now GPIO1 (Pico GP28), the hardware plan's confirmed spare. Verified on real hardware: `"RDM6300 UART initialized on GPIO1"` now prints and boot continues cleanly through Wi-Fi STA and BLE bring-up, no watchdog reboot. Evidence: `main/rfid/rdm6300.c` (`rdm6300_init`), `main/main.c` (`rdm6300_init()` call restored, no longer commented out).
