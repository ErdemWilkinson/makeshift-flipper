...
- :red_circle: **CONFIRMED — THE PHYSICAL BACK BUTTON DOES NOTHING IN MENUS:** `buttons.h` defines `BUTTON_BACK` as the standalone button that “always means back”, but the main menu loop passes it to `menu_handle_button()` and that switch handles only `BUTTON_LEFT` for parent navigation.  Therefore BACK is ignored at the top level and in every submenu; only joystick-left exits a submenu.  This conflicts with the input contract and with the otherwise consistent use of BACK to leave action screens.  Treat `BUTTON_BACK` as `BUTTON_LEFT` in `menu_handle_button()` (or translate it before the call) so the hardware back button navigates to the parent menu.

- :red_circle: **CONFIRMED — WI-FI SETUP CAN TRAP THE UI FOR SIX MINUTES WITH NO CANCEL PATH:** `action_wifi_setup()` calls synchronous `c6_link_setup()`, whose `read_line()` loop waits up to `SETUP_TIMEOUT_MS` (six minutes) and never polls buttons.  On the C6, `wifi_setup_ap_run()` likewise blocks the command-dispatch loop for its five-minute wait; it exposes no `SETUPSTOP` command.  Pressing the physical BACK button while the AP/password instructions are displayed has no effect, and no other C6 feature can be used until timeout or a successful phone submission.  Add an interruptible P4 wait plus a C6 `SETUPSTOP` command that tears down the server/AP cleanly, and display `BACK: cancel` only once it actually works.  Evidence: `main/main.c` (`action_wifi_setup`), `main/net/c6_link.c` (`c6_link_setup_impl`), `c6-firmware/main/main.c`, and `c6-firmware/main/wifi_setup_ap.c`.

- :large_orange_circle: **CONFIRMED — C6 SCAN FAILURES ARE PRESENTED AS “NO NETWORKS FOUND”:** `wifi_commands_scan()` emits the identical `SCANDONE` response for Wi-Fi initialization failure, `esp_wifi_scan_start()` failure, AP-record retrieval failure, allocation failure, and a genuine empty scan.  `c6_link_scan()` consequently returns `0` for every one of those C6 failures.  `action_wifi_setup_manual()` displays “No networks found”, and the scan-test screen also displays its empty-state result, concealing a broken radio/driver from a device-only user.  Introduce a distinct wire response such as `SCANFAIL:<reason>`, propagate it as a negative result, and show it as an error.  Evidence: `c6-firmware/main/wifi_commands.c` (`wifi_commands_scan`), `main/net/c6_link.c` (`c6_link_scan_impl`), and `main/main.c` (both scan callers).

- :yellow_circle: **CONFIRMED — WI-FI SCAN'S “MORE RESULTS” INDICATOR IS DRAWN THEN OVERWRITTEN:** In `action_wifi_scan_test()`, the first 12 SSIDs occupy display rows 2–13.  When more exist, `"(+N more, see logs)"` is drawn at row `2 + shown` = 14, then the unconditional `"Press any key"` footer is drawn to `DISPLAY_ROWS - 1` = 14.  The indicator is never visible, despite the code/comment claiming it is; a user cannot tell the shown list was truncated.  Reserve a distinct row for the footer and truncation message, or make this view scrollable.  Evidence: `main/main.c` (`action_wifi_scan_test` with `LIST_VISIBLE_ROWS`, `shown`, and the row-14 footer).

- :red_circle: **CONFIRMED — CLONE CAN WRITE INVALID/UNINITIALIZED SOURCE BLOCK 0 TO A MAGIC TARGET:** `dump_card()` marks a sector unreadable when authentication or any block read fails.  In the authentication-failure path it zeros that sector's blocks; in a post-authentication read-failure path the remaining block data can be uninitialized because the heap-allocated dump was not zeroed.  Nevertheless, `action_rfid_clone()` copies `dump->sectors[0].blocks[0]` and calls `rc522_gen1a_write_block0()` whenever the target UID length matches, without requiring `dump->sectors[0].readable`.  A Gen1a “magic” target accepts that backdoor write, so a source whose sector 0 was not completely read can have its target manufacturer block replaced with zeroed or indeterminate bytes.  Do not offer/attempt the UID-clone pass unless sector 0 and block 0 were read successfully; initialize the whole dump before acquisition and show a clear “UID not read; skipped” status.  Evidence: `main/main.c` (`dump_card`, `action_rfid_clone`) and `main/rfid/rc522.c` (`rc522_gen1a_write_block0`).

- :large_orange_circle: **CONFIRMED — LIBRARY DELETES ARE ONE-KEY, PERMANENT, AND HAVE NO CONFIRMATION/UNDO:** Both saved-RFID and saved-IR browsers map a single `BUTTON_LEFT` event directly to `*_library_remove(selected)` and immediately persist the change.  There is no confirmation, undo grace period, or recovery view.  This is especially easy to trigger because joystick-left is the normal menu-navigation/back gesture elsewhere in the UI, and erases a user-created record from NVS.  Require a confirm/cancel screen (or at least delayed commit with undo) before persisting deletion.  Evidence: `main/main.c` (`action_rfid_library` and `action_ir_library`).

- :large_orange_circle: **CONFIRMED — A PERSISTENT RC522 FAULT FLOODS AND ERASES THE DIAGNOSTIC HISTORY:** The normal 13.56MHz scan screen calls `diag_record_error("13.56MHz NFC", "RC522_SCAN_ERROR")` on every approximately 10ms `RC522_SCAN_ERROR` result.  Unlike the held-card/unsupported-UID path, it has no edge or rate limiter.  An unplugged/broken SPI reader can therefore overwrite all 24 diagnostic slots in about 240ms, hiding the preceding failures the Errors menu is meant to preserve.  Record the transition into an error state once, or rate-limit identical scan errors until a successful/no-card state resets it.  Evidence: `main/main.c` main scan loop (`RC522_SCAN_ERROR` branch) and `main/diag/diag.h` (`DIAG_HISTORY_CAPACITY`).

- :yellow_circle: **CONFIRMED — “SAVE 13.56MHz TAG” RECORDS ITS UNSUPPORTED-UID ERROR UNDER THE WRONG FEATURE:** The reusable `wait_for_card()` helper always calls `diag_record_error("RFID Clone", "RC522_SCAN_UNSUPPORTED_UID")`.  It is also called by `action_rfid_save_1356mhz()`, so an unsupported tag encountered while saving is misleadingly shown in Errors as a clone failure.  Pass a diagnostic module identifier into the helper or have each caller record its own contextual error.  Evidence: `main/main.c` (`wait_for_card`, `action_rfid_clone`, and `action_rfid_save_1356mhz`).
