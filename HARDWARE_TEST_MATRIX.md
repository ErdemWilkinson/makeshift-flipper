...
| "WiFi Monitor" shows real nearby beacon/probe-response traffic, correctly channel-hopping 1-13 | AP list changes as you move / as nearby APs change channel | **Known gap, see KNOWN_ISSUES.md Round 27:** no Wi-Fi country is configured and per-channel hop failures aren't checked, so a channel your regulatory domain disallows can silently be skipped while the UI still looks like it's hopping normally. |
