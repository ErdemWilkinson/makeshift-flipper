...
**Date:** 2026-09-21  
**Scope:** This document is a design and compliance assessment based on the
single-MCU ESP32-C6 firmware (UI + on-chip Wi-Fi/BLE radio) and the existing
hardware plans in this repository. It is not legal advice or an on-device
validation report.

> **Architecture note (2026-09-23):** The device was previously a two-chip
> design (ESP32-P4 main MCU + ESP32-C6 radio companion over UART). It is now
> a single ESP32-C6 running both the UI and the radio directly. References
> below to "the P4", "the C6 companion", or "the P4↔C6 UART link" describe the
> superseded design; in the current build those are one chip with no inter-MCU
> link, which removes the UART-boundary and cross-MCU-trust risks noted in
> those places rather than adding new ones.
...
> validated on physical hardware. The firmware builds clean for the ESP32-C6,
> but the hardware has not been assembled — the ST7789 display wiring, the
> shared SPI bus, the RIGHT-button/I2C-SDA sharing, and Wi-Fi+BLE coexistence
> are all unverified. Therefore, “present” below means in code/design scope,
> not “working.”
...
| 3. Pocket wireless site survey | See nearby authorized Wi-Fi APs and BLE advertisers, with signal and channel information | On-chip ESP32-C6 Wi-Fi/BLE radios, passive monitor/scan UI | Collecting or profiling nearby networks/devices where there is no authority | Receive-only, short-lived display, no raw identifier saving by default. |
| 4. Offline field diagnostics | Carry errors and module status on-device for local debugging | On-device rolling error history (NVS) | Retaining scan/card data about people or networks with no authorization | On-device only; the old LAN log export is not part of the standalone build. |
| 5. No-phone Wi-Fi onboarding | Join an owned network by scanning and entering a password with the joystick keyboard, without a built-in keyboard | On-chip Wi-Fi STA scan + connect, display, joystick text entry | Mishandling or exposing credentials in an insecure/unauthorized setup environment | Local entry only; the old web-based Setup AP is not yet in the standalone build. |
| 6. Freed lab/TinyML platform | The former P4 main MCU is now free for a separate OCR/voice TinyML project, not carried in the field | (separate project; not this firmware) | Covert image/audio collection if a hostile build removes consent/indicators | Kept out of the field device entirely; evaluated in its own project. |
...
and an on-chip radio on a single MCU, not because it disrupts, impersonates, or secretly
...
1. (No longer applicable in the standalone build: the web-based Wi-Fi Setup
   AP and the plain-HTTP error-log endpoint are not part of it. If either is
   ported back, add authentication and encryption first.)
2. Keep sanitizing untrusted SSID/BLE-name text on screen (control/comma/CR/
   LF stripped) so a hostile name can't corrupt on-device rendering; the old
   UART-boundary version of this risk is gone with the wire protocol.
...
**ESP32-C6 main MCU (single chip)** — runs the local UI, input, IR,
RFID/NFC, diagnostics, AND the Wi-Fi/BLE radio, all on one chip.
...
whatever else is attached. Because the radio now lives on the same chip,
this MCU also carries the RF risks listed below directly.
*Limit:* only this repository's firmware choices keep the radio receive-only;
the silicon supports more.
...
**On-chip Wi-Fi/BLE radio (same ESP32-C6)** — performs Wi-Fi scan/monitor
and BLE scanning today, receive-only by design.
...
**On-chip BLE radio** — passive advertisement discovery of nearby BLE
devices today, never connects.
*Already in the design; different firmware on it could enable:* active
...
**(Removed) P4↔C6 UART link** — in the two-chip design this was a wired
command/result channel between the P4 and the C6. The single-MCU build has
no such link: the UI and the radio are the same chip and call each other as
in-process functions.
*Effect on risk:* the old cross-MCU risks — passing surveillance data
between two processors invisibly, or one side disabling the safety/
confirmation state the other enforces — no longer apply, because there is
no second processor and no wire to desynchronize. The UI code and the radio
code share one address space; their consistency is a code-review property,
not a wire-protocol one.
*Required boundary:* keep the UI's confirmation/authorization gates in the
same firmware image that drives the radio, and cover them in review.
