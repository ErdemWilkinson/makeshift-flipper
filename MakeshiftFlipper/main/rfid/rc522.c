...
#define PIN_RST GPIO_NUM_2 // Pico GP27

#define SPI_HOST_USED SPI2_HOST // shared with the LCD; bus init done by display.c
...
{
    // The SPI2 bus is already initialized by display_init(); this device
    // only joins it. Give the RC522 a real hardware reset before issuing
    // the protocol-level soft reset.
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "reset GPIO init failed; RC522 disabled: %s",
                 esp_err_to_name(err));
        s_spi_fault = true;
        return;
    }
    ...
    err = spi_bus_add_device(SPI_HOST_USED, &dev_cfg, &s_spi);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "shared SPI attach failed; RC522 disabled: %s",
                 esp_err_to_name(err));
        s_spi = NULL;
        s_spi_fault = true;
        return;
